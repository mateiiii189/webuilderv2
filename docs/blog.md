...
The listing at `/blog` starts with four articles, newest first. **Arată mai multe** appends four, with the existing reveal animation. Each article has its own `/blog/[slug]` page, reading-time estimate, table of contents and contact CTA. Article pages use the full site content width, with a compact cover on the left and the title/introduction on the right above 900px. On smaller screens the title stays above the cover. The article body begins below this header. At 1101px and above, the contents list stays in a sticky right column. Below that width, a fixed bottom-right **Cuprins** menu opens the section links without taking space above the article. It closes on selection, outside click or Escape; Escape returns focus to the control.
