...
export default function ContactForm({ email }: { email: string }) {
  ...
  function prepareEmail(event: FormEvent<HTMLFormElement>) {
    ...
    const subject = meeting ? "Discuție online" : value("service");
    const body = [
      `Nume: ${value("name")}`,
      `E-mail: ${value("email")}`,
      ...
      ...(meeting
        ? [
            "Întâlnire online de 30 de minute, prin Google Meet.",
            ...(value("availability")
              ? [`Disponibilitate preferată: ${value("availability")}`]
              : []),
          ]
        : []),
      "",
  ...
  return (
    <form
      ...
      className="contact-form rounded-card border border-border bg-surface p-5 sm:p-8 xl:p-10"
    ...
    >
      ...
      <div className="mt-7 mb-7" aria-live="polite">
        <h2 className="text-[clamp(24px,2.3vw,34px)] leading-tight font-medium tracking-heading">
          {meeting ? "Hai să vorbim despre idee." : "Ce ai vrea să construim?"}
        </h2>
        <p className="mt-3 text-sm leading-relaxed text-muted">
          {meeting
            ? "Lasă-ne câteva detalii. Stabilim data prin e-mail înainte să confirmăm întâlnirea."
      ...
      <fieldset hidden={meeting} disabled={meeting} className="mb-7 min-w-0">
        <legend className="mb-3 text-[13px] text-label">Mă interesează</legend>
      ...
      <fieldset hidden={!meeting} disabled={!meeting} className="min-w-0">
        <FormField
          id="contact-availability"
          label="Când ți-ar fi comod?"
          optional
        >
          <Input
            id="contact-availability"
            name="availability"
            placeholder="De exemplu, marți după ora 14:00"
            maxLength={160}
            aria-describedby="meeting-hours"
          />
          <p
            id="meeting-hours"
            className="mt-2 text-xs leading-relaxed text-muted"
          >
            Luni–vineri, 10:00–18:00 · ora Bucureștiului. Confirmăm
            disponibilitatea prin e-mail.
          </p>
        </FormField>
      </fieldset>
